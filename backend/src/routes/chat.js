...
router.delete('/:messageId', authMiddleware, async (req, res) => {
  try {
    ...
    // Find the message
    const [msg] = await db.select().from(chatMessages).where(eq(chatMessages.id, parseInt(messageId)));
    ...
    if (req.user.role !== 'admin') {
      // User can only delete their own message (not admin replies, and must match userId)
      if (msg.userId !== req.user.id || msg.isFromAdmin) {
        return res.status(403).json({ message: 'Anda tidak memiliki akses untuk menghapus pesan ini' });
      }
    }

    // Delete the message
    await db.delete(chatMessages).where(eq(chatMessages.id, parseInt(messageId)));

    // Emit socket event
    if (req.io) {
      req.io.to('admin_room').emit('deleteMessage', parseInt(messageId));
      req.io.to(`user_${msg.userId}`).emit('deleteMessage', parseInt(messageId));
    }
